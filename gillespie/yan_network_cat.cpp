 ...
 */
void yan_network_init(BasicGillespie &BG, double prot_lifetime) {
  // General Constants
  ...
  // Protein
  double DEF_PRT_DEG = prot_lifetime;
...
int main(int argc, char* argv[]) {
  ...
  // check that number is correct
   if (argc != 2) {
    std::cerr << "Usage: " << argv[0] << " Parameter setting (1 = loop protein lifetimes)" << std::endl;
    return 1;
  }
  int parameter_setting = atoi(argv[1]);
  ...
  // Parameter setting 1: Loop protein lifetimes from 30 to 120 minutes
  if (parameter_setting == 1) {
    // vector<double> p_lifetimes = {30.0, 120.0, 600.0}
    // for (auto i = lifetimes.begin(); i != lifetimes.end(); ++i) {
    for (int j=60; j<=660; j+=120) {
      double prot_lifetime = 1.0/(60*j);
    ...
    std::cerr << "Only parameter setting 1 currently available" << std::endl;
  }
}
